...
def decide_live_boundary_band(
    ...
    band_live = bool(
        np.isfinite(boundary_frac) and np.isfinite(corr_ratio) and np.isfinite(effw_ratio)
        and (boundary_frac >= float(trigger_frac))
        and (corr_ratio >= float(trigger_corr))
    ...
    if operator == "wls":
        ...
        for attempt_idx, (alpha_cc, min_cc, max_cc) in enumerate(attempt_configs):
            ...
            decision = decide_live_boundary_band(
                ...
                trigger_corr=live_boundary_trigger_corr,
...
def aggregate_rows(rows: List[Dict[str, float]]) -> Tuple[List[Dict[str, float]], List[Dict[str, float]]]:
    ...
    for key, grp in sorted(groups.items()):
        ...
        keys_to_average = [
            ...
            "live_boundary_band_patch_quad_tol",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--live-boundary-band-factor", type=float, default=2.0, help="Boundary-band width as a multiple of target h.")
    p.add_argument("--live-boundary-min-scale", type=float, default=0.80, help="Minimum original-edge prior-weight scale at the boundary when the live band is active.")
    p.add_argument("--live-boundary-trigger-frac", type=float, default=0.30, help="Turn the live boundary band on only if the original boundary-band edge fraction exceeds this value.")
    p.add_argument("--live-boundary-trigger-corr", type=float, default=1.20, help="Turn the live boundary band on only if boundary corr p90 / original-core corr p90 exceeds this value.")
    p.add_argument("--live-boundary-trigger-effw-ratio", type=float, default=3.50, help="Turn the live boundary band on only if effW(boundary) p90 / effW(boundary) median exceeds this value.")
