...
def classify_ccpfm_edges(
    ...
    boundary_band_width: float | None = None,
) -> Dict[str, Array]:
    m = len(edges)
    ...
    band_threshold = float(boundary_band_width) if boundary_band_width is not None else core_threshold
    for k, (i, j) in enumerate(edges):
        ii = not bool(is_boundary[i])
        jj = not bool(is_boundary[j])
        both_interior[k] = ii and jj
        di = 0.0 if bool(is_boundary[i]) else float(dist_to_boundary[i])
        dj = 0.0 if bool(is_boundary[j]) else float(dist_to_boundary[j])
        if ii and jj and (dist_to_boundary[i] > core_threshold) and (dist_to_boundary[j] > core_threshold):
            is_core[k] = True
        if min(di, dj) <= band_threshold:
...
def make_live_boundary_edge_mask(
    edges: List[Tuple[int, int]],
    is_boundary: Array,
    dist_to_boundary: Array,
    band_width: float,
    ...
    m = len(edges)
    edge_live_band = np.zeros(m, dtype=bool)
    edge_boundary_distance = np.full(m, np.nan, dtype=float)
    bw = max(float(band_width), 1.0e-14)
    for k, (i, j) in enumerate(edges):
        if bool(edge_is_added[k]):
            continue
        di = 0.0 if bool(is_boundary[i]) else float(dist_to_boundary[i])
        dj = 0.0 if bool(is_boundary[j]) else float(dist_to_boundary[j])
        de = min(di, dj)
        edge_boundary_distance[k] = de
        if de <= bw:
            edge_live_band[k] = True
    return edge_live_band, edge_boundary_distance


def apply_live_boundary_band_scaling(
    weights: Array,
    edge_live_band: Array,
    edge_boundary_distance: Array,
    band_width: float,
    min_scale: float,
) -> Tuple[Array, Array]:
    weights_live = np.asarray(weights, dtype=float).copy()
    edge_live_scale = np.ones(len(weights_live), dtype=float)
    bw = max(float(band_width), 1.0e-14)
    smin = min(max(float(min_scale), 1.0e-12), 1.0)
    for k in np.where(edge_live_band)[0]:
        de = float(edge_boundary_distance[k])
        taper = min(max(de / bw, 0.0), 1.0)
        scale = smin + (1.0 - smin) * taper
        edge_live_scale[k] = scale
        weights_live[k] *= scale
    return weights_live, edge_live_scale


def decide_live_boundary_band(
    corr_stats: Dict[str, float],
    h_target: float,
    trigger_frac: float,
    trigger_boundary_corr: float,
    trigger_effw_ratio: float,
    h_max: float,
) -> Dict[str, float | bool]:
    eps = 1.0e-12
    boundary_frac = float(corr_stats.get('original_boundary_edge_fraction', float('nan')))
    boundary_corr_p90 = float(corr_stats.get('original_boundary_edge_correction_rel_p90', float('nan')))
    core_corr_p90 = float(corr_stats.get('original_core_edge_correction_rel_p90', float('nan')))
    boundary_effw_med = float(corr_stats.get('original_boundary_effective_weight_median', float('nan')))
    boundary_effw_p90 = float(corr_stats.get('original_boundary_effective_weight_p90', float('nan')))

    corr_ratio = boundary_corr_p90 / max(core_corr_p90, eps) if np.isfinite(boundary_corr_p90) and np.isfinite(core_corr_p90) else float('nan')
    effw_ratio = boundary_effw_p90 / max(boundary_effw_med, eps) if np.isfinite(boundary_effw_p90) and np.isfinite(boundary_effw_med) else float('nan')
    late_enough = bool(np.isfinite(h_target) and (float(h_target) <= float(h_max)))
    band_live = bool(
        late_enough
        and np.isfinite(boundary_frac)
        and np.isfinite(boundary_corr_p90)
        and np.isfinite(effw_ratio)
        and (boundary_frac >= float(trigger_frac))
        and (boundary_corr_p90 >= float(trigger_boundary_corr))
        and (effw_ratio >= float(trigger_effw_ratio))
    )
    return {
        'band_live': band_live,
        'late_enough': late_enough,
        'boundary_frac': boundary_frac,
        'boundary_corr_p90': boundary_corr_p90,
        'core_corr_p90': core_corr_p90,
        'corr_ratio': corr_ratio,
        'boundary_effw_med': boundary_effw_med,
        'boundary_effw_p90': boundary_effw_p90,
        'effw_ratio': effw_ratio,
...
def solve_ccpfm_correction(
    ...
    if edge_classes is not None:
        ...
        def _subset_weight_stats(mask: Array) -> Tuple[float, float]:
            if mask is None or np.sum(mask) == 0:
                return float('nan'), float('nan')
            ww = weights_eff[mask]
            return float(np.median(ww)), float(np.quantile(ww, 0.90))
        ...
        bnd_w_med, bnd_w_p90 = _subset_weight_stats(edge_touches_boundary_band)
        orig_bnd_w_med, orig_bnd_w_p90 = _subset_weight_stats(orig_bnd)
        n_all = max(len(lambda0), 1)
        n_orig = max(int(np.sum(edge_is_original)), 1)
        stats.update({
            "boundary_edge_fraction": float(np.sum(edge_touches_boundary_band) / n_all),
            "original_boundary_edge_fraction": float(np.sum(orig_bnd) / n_orig),
            ...
            "boundary_effective_weight_median": bnd_w_med,
            "boundary_effective_weight_p90": bnd_w_p90,
            ...
            "original_boundary_effective_weight_median": orig_bnd_w_med,
            "original_boundary_effective_weight_p90": orig_bnd_w_p90,
...
def assemble_operator_and_diagnostics(
    ...
    h_target: float,
    live_boundary_band: bool,
    live_boundary_band_factor: float,
    live_boundary_min_scale: float,
    live_boundary_trigger_frac: float,
    live_boundary_trigger_corr: float,
    live_boundary_trigger_effw_ratio: float,
    live_boundary_hmax: float,
    live_boundary_patch_quad_tol: float,
    ...
    if operator == "wls":
        ...
        boundary_band_width = max(float(live_boundary_band_factor) * float(h_target), 1.0e-14)
        for attempt_idx, (alpha_cc, min_cc, max_cc) in enumerate(attempt_configs):
            ...
            edge_classes = classify_ccpfm_edges(
                edges, is_boundary, dist_to_boundary,
                h_ref=h_ref,
                edge_is_added=edge_is_added,
                core_factor=selective_core_factor,
                boundary_band_width=boundary_band_width,
            )
            edge_live_boundary_band, edge_boundary_distance = make_live_boundary_edge_mask(
                edges=edges,
                is_boundary=is_boundary,
            ...
            lambdas_frozen, corr_stats_frozen = solve_ccpfm_correction(
            ...
            A_frozen = build_ccpfm_matrix(points, is_boundary, adjacency, edge_index, lambdas_frozen, volumes)
            patches_frozen = patch_test_metrics(A_frozen, points, is_boundary)

            decision = decide_live_boundary_band(
                corr_stats=corr_stats_frozen,
                h_target=h_target,
                trigger_frac=live_boundary_trigger_frac,
                trigger_boundary_corr=live_boundary_trigger_corr,
                trigger_effw_ratio=live_boundary_trigger_effw_ratio,
                h_max=live_boundary_hmax,
            )
            live_info = {
                "live_boundary_band_requested": bool(live_boundary_band),
                "live_boundary_band_triggered": bool(decision["band_live"]),
                "live_boundary_band_active": False,
                "live_boundary_band_reverted": False,
                "live_boundary_band_width": float(boundary_band_width),
                "live_boundary_band_min_scale": float(live_boundary_min_scale),
                "live_boundary_band_boundary_frac": float(decision["boundary_frac"]),
                "live_boundary_band_boundary_corr_p90": float(decision["boundary_corr_p90"]),
                "live_boundary_band_orig_boundary_corr_p90": float(decision["boundary_corr_p90"]),
                "live_boundary_band_core_corr_p90": float(decision["core_corr_p90"]),
                "live_boundary_band_corr_ratio": float(decision["corr_ratio"]),
                "live_boundary_band_boundary_effw_med": float(decision["boundary_effw_med"]),
                "live_boundary_band_boundary_effw_p90": float(decision["boundary_effw_p90"]),
                "live_boundary_band_effw_ratio": float(decision["effw_ratio"]),
                "live_boundary_band_patch_quad_after": float('nan'),
                "live_boundary_band_trigger_frac": float(live_boundary_trigger_frac),
                "live_boundary_band_trigger_corr": float(live_boundary_trigger_corr),
                "live_boundary_band_trigger_effw_ratio": float(live_boundary_trigger_effw_ratio),
                "live_boundary_band_hmax": float(live_boundary_hmax),
                "live_boundary_band_late_enough": bool(decision["late_enough"]),
                "live_boundary_band_patch_quad_tol": float(live_boundary_patch_quad_tol),
            }

            lambdas = lambdas_frozen
            corr_stats = corr_stats_frozen
            A = A_frozen
            patches_tmp = patches_frozen
            edge_live_scale = np.ones(len(weights), dtype=float)

            if bool(live_boundary_band) and bool(decision["band_live"]):
                weights_live, edge_live_scale = apply_live_boundary_band_scaling(
                    weights=weights,
                    edge_live_band=edge_live_boundary_band,
                    edge_boundary_distance=edge_boundary_distance,
                    band_width=boundary_band_width,
                    min_scale=live_boundary_min_scale,
                )
                lambdas_live, corr_stats_live = solve_ccpfm_correction(
                    lambda0, weights_live, Acons, bcons, regularization=constraint_regularization,
                    edge_classes=edge_classes,
                    original_prior_anchor=original_prior_anchor,
                    core_prior_anchor=core_prior_anchor,
                    boundary_band_prior_anchor=boundary_band_prior_anchor,
                ...
                A_live = build_ccpfm_matrix(points, is_boundary, adjacency, edge_index, lambdas_live, volumes)
                patches_live = patch_test_metrics(A_live, points, is_boundary)
                live_info["live_boundary_band_patch_quad_after"] = float(patches_live["patch_quad_max"])
                if float(patches_live["patch_quad_max"]) <= float(live_boundary_patch_quad_tol):
                    lambdas = lambdas_live
                    corr_stats = corr_stats_live
                    A = A_live
                    patches_tmp = patches_live
                    live_info["live_boundary_band_active"] = True
                else:
                    live_info["live_boundary_band_reverted"] = True
            ...
            if score < best_score:
                best_score = score
                best = (
                    A, adjacency, edges, edge_index, local_conds, local_sigmins, lambda0, lambdas, distances,
                    corr_stats, patches_tmp, meta, edge_is_added, node_is_flagged, node_is_force_flagged,
                    node_added_count, attempt_idx, alpha_cc, min_cc, max_cc,
                    edge_live_boundary_band, edge_live_scale, live_info,
                )
            if score <= 5.0e-7:
                break
        if best is None:
        ...
        (
            A, adjacency, edges, edge_index, local_conds, local_sigmins, lambda0, lambdas, distances,
            corr_stats, patches, meta, edge_is_added, node_is_flagged, node_is_force_flagged,
            node_added_count, selected_attempt_idx, selected_attempt_alpha, selected_attempt_min_neighbors,
            selected_attempt_max_neighbors, edge_live_boundary_band, edge_live_scale, live_info,
        ) = best
        ...
        diagnostics_extra = {
            ...
            "edge_is_added": edge_is_added,
            "edge_live_boundary_band": edge_live_boundary_band,
            "edge_live_boundary_scale": edge_live_scale,
...
def solve_poisson_case(
    ...
    A, b, stats, diagnostics = assemble_operator_and_diagnostics(
        ...
        h_target=h,
        live_boundary_band=live_boundary_band,
        live_boundary_band_factor=live_boundary_band_factor,
        live_boundary_min_scale=live_boundary_min_scale,
        live_boundary_trigger_frac=live_boundary_trigger_frac,
        live_boundary_trigger_corr=live_boundary_trigger_corr,
        live_boundary_trigger_effw_ratio=live_boundary_trigger_effw_ratio,
        live_boundary_hmax=live_boundary_hmax,
        live_boundary_patch_quad_tol=live_boundary_patch_quad_tol,
    )
    ...
    if save_points:
        np.savez_compressed(
            ...
            edge_live_boundary_band=diagnostics.get("edge_live_boundary_band", np.array([], dtype=bool)),
            edge_live_boundary_scale=diagnostics.get("edge_live_boundary_scale", np.array([], dtype=float)),
...
def aggregate_rows(rows: List[Dict[str, float]]) -> Tuple[List[Dict[str, float]], List[Dict[str, float]]]:
    ...
    for key, grp in sorted(groups.items()):
        ...
        keys_to_average = [
            ...
            "original_edge_fraction", "added_edge_fraction", "boundary_edge_fraction", "original_boundary_edge_fraction",
            "n_added_edges", "n_original_edges", "n_flagged_enrichment_nodes",
            ...
            "boundary_effective_weight_median", "boundary_effective_weight_p90",
            "original_core_edge_correction_rel_median", "original_core_edge_correction_rel_p90", "original_core_edge_correction_rel_max", "original_core_edge_negative_lambda_fraction",
            "original_boundary_edge_correction_rel_median", "original_boundary_edge_correction_rel_p90", "original_boundary_edge_correction_rel_max", "original_boundary_edge_negative_lambda_fraction",
            "original_boundary_effective_weight_median", "original_boundary_effective_weight_p90",
            "live_boundary_band_requested", "live_boundary_band_triggered", "live_boundary_band_active", "live_boundary_band_reverted",
            "live_boundary_band_width", "live_boundary_band_min_scale", "live_boundary_band_boundary_frac",
            "live_boundary_band_boundary_corr_p90", "live_boundary_band_orig_boundary_corr_p90", "live_boundary_band_core_corr_p90",
            "live_boundary_band_corr_ratio", "live_boundary_band_boundary_effw_med", "live_boundary_band_boundary_effw_p90",
            "live_boundary_band_effw_ratio", "live_boundary_band_patch_quad_after",
            "live_boundary_band_trigger_frac", "live_boundary_band_trigger_corr", "live_boundary_band_trigger_effw_ratio",
            "live_boundary_band_hmax", "live_boundary_band_late_enough", "live_boundary_band_patch_quad_tol",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--live-boundary-band", action="store_true", help="Enable the live boundary-band prior softening branch on top of the frozen prior-preserving graph.")
    p.add_argument("--live-boundary-band-factor", type=float, default=1.0, help="Boundary-band width as a multiple of target h. Recommended thin live branch: 1.0.")
    p.add_argument("--live-boundary-min-scale", type=float, default=0.90, help="Minimum original-edge prior-weight scale at the boundary when the live band is active. Recommended mild live branch: 0.90.")
    p.add_argument("--live-boundary-trigger-frac", type=float, default=0.60, help="Turn the live boundary band on only if the original boundary-band edge fraction exceeds this value.")
    p.add_argument("--live-boundary-trigger-corr", type=float, default=2.00, help="Turn the live boundary band on only if original boundary corr p90 exceeds this value.")
    p.add_argument("--live-boundary-trigger-effw-ratio", type=float, default=5.00, help="Turn the live boundary band on only if effW(boundary) p90 / effW(boundary) median exceeds this value.")
    p.add_argument("--live-boundary-hmax", type=float, default=0.12, help="Turn the live boundary band on only for h <= this value.")
    p.add_argument("--live-boundary-patch-quad-tol", type=float, default=5.0e-8, help="Revert the live boundary band for a case if the post-live quadratic patch residual exceeds this tolerance.")
...
def main(argv: Sequence[str] | None = None) -> int:
                ...
                for op in operators:
                    tasks.append(
                        dict(
                            ...
                            live_boundary_band=bool(args.live_boundary_band),
                            live_boundary_band_factor=float(args.live_boundary_band_factor),
                            live_boundary_min_scale=float(args.live_boundary_min_scale),
                            live_boundary_trigger_frac=float(args.live_boundary_trigger_frac),
                            live_boundary_trigger_corr=float(args.live_boundary_trigger_corr),
                            live_boundary_trigger_effw_ratio=float(args.live_boundary_trigger_effw_ratio),
                            live_boundary_hmax=float(args.live_boundary_hmax),
                            live_boundary_patch_quad_tol=float(args.live_boundary_patch_quad_tol),
            ...
            try:
                ...
                if row["operator"] == "ccpfm":
                    extra_msg = (
                        ...
                        f" bandlive={int(bool(row.get('live_boundary_band_active', False)))}"
                    )
                print(
    ...
    for r in sorted([a for a in agg_rows if a['operator'] == 'ccpfm'], key=lambda z: z['domain']):
        if float(r['h_target']) == min(args.hs):
            band_live_txt = "YES" if float(r.get('mean_live_boundary_band_active', 0.0)) >= 0.5 else "NO"
            print(
                f"{r['domain']:>10s}: h={r['h_target']:.4f}, median L2={r['median_l2']:.3e}, "
                f"median Linf={r['median_linf']:.3e}, global order(L2, median)={r['global_order_l2_median']:.3f}, "
                ...
                f"added frac={r.get('mean_added_edge_fraction', float('nan')):.3f}, boundary frac={r.get('mean_live_boundary_band_boundary_frac', float('nan')):.3f}, "
                f"orig corr p90={r.get('mean_original_edge_correction_rel_p90', float('nan')):.2f}, orig-core corr p90={r.get('mean_original_core_edge_correction_rel_p90', float('nan')):.2f}, "
                f"boundary corr p90={r.get('mean_live_boundary_band_boundary_corr_p90', float('nan')):.2f}, orig-boundary corr p90={r.get('mean_live_boundary_band_orig_boundary_corr_p90', float('nan')):.2f}, "
                f"effW(boundary) p90={r.get('mean_live_boundary_band_boundary_effw_p90', float('nan')):.2e}, attempt={r.get('mean_selected_attempt_idx', float('nan')):.2f}, band-live={band_live_txt}"
            )

    print("\nBoundary-band diagnostic summary (mean values at finest h):")
    for r in sorted([a for a in agg_rows if a['operator'] == 'ccpfm' and float(a['h_target']) == min(args.hs)], key=lambda z: z['domain']):
        band_live_txt = "YES" if float(r.get('mean_live_boundary_band_active', 0.0)) >= 0.5 else "NO"
        print(
            f"  {r['domain']:>8s}: boundary frac={r.get('mean_live_boundary_band_boundary_frac', float('nan')):.3f}, "
            f"orig-boundary frac={r.get('mean_original_boundary_edge_fraction', float('nan')):.3f}, "
            f"boundary corr p90={r.get('mean_live_boundary_band_boundary_corr_p90', float('nan')):.2f}, "
            f"orig-boundary corr p90={r.get('mean_live_boundary_band_orig_boundary_corr_p90', float('nan')):.2f}, "
            f"effW(boundary) med={r.get('mean_live_boundary_band_boundary_effw_med', float('nan')):.2e}, "
            f"effW(boundary) p90={r.get('mean_live_boundary_band_boundary_effw_p90', float('nan')):.2e}, "
            f"band-live={band_live_txt}"
        )
