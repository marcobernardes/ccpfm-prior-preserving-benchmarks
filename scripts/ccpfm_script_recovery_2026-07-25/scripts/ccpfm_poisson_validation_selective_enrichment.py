...
def build_edge_list(
...
) -> Tuple[List[Tuple[int, int]], Dict[Tuple[int, int], int], List[List[int]], Array, Array, Array, Array]:
    ...
    *very sparse* rescue edges only for flagged interior-core nodes.
    ...
    Returns edge_is_added and node_is_flagged arrays so diagnostics can separate
    original edges from added rescue edges.
    """
    tree = cKDTree(points)
    ...
    if selective_enrichment_neighbors > 0:
        ...
        if interior_idx.size:
            ...
            for i in interior_idx:
                ...
                kq = min(len(interior_idx), len(adjacency_nodes[i]) + 20)
                d, q = tree_int.query(points[i], k=kq)
                q = np.atleast_1d(q)
                d = np.atleast_1d(d)
                added = 0
                dist_cap = selective_distance_factor * max(radii[i], median_r)
    ...
    return edges, edge_index, adjacency, local_conds, local_sigmins, edge_is_added, node_is_flagged
...
def compute_ccpfm_prior(
    ...
    """Hybrid prior: original edges blend geometric and WLS-derived suggestions,
    while added rescue edges keep a geometric prior and are heavily penalized in
    the correction objective so they act as last-resort degrees of freedom.
    """
    ...
    lambda0[use_blend] = 0.35 * lambda_geom[use_blend] + 0.65 * lambda_wls[use_blend]
...
def solve_ccpfm_correction(
    ...
    winv = 1.0 / np.maximum(weights, 1.0e-16)
    ...
    if edge_classes is not None:
        core_stats = _subset_rel_stats(delta, lambda0, edge_classes.get('edge_is_core'))
        bnd_stats = _subset_rel_stats(delta, lambda0, edge_classes.get('edge_touches_boundary_band'))
        add_stats = _subset_rel_stats(delta, lambda0, edge_classes.get('edge_is_added'))
        orig_stats = _subset_rel_stats(delta, lambda0, edge_classes.get('edge_is_original'))
        n_all = max(len(lambda0), 1)
        stats.update({
            "original_edge_fraction": float(np.sum(edge_classes.get('edge_is_original', np.zeros(0, dtype=bool))) / n_all),
            "added_edge_fraction": float(np.sum(edge_classes.get('edge_is_added', np.zeros(0, dtype=bool))) / n_all),
...
def assemble_operator_and_diagnostics(
    ...
    if operator == "wls":
        ...
        for alpha_cc, min_cc, max_cc in attempt_configs:
            edges, edge_index, adjacency, local_conds, local_sigmins, edge_is_added, node_is_flagged = build_edge_list(
            ...
            lambdas, corr_stats = solve_ccpfm_correction(
                lambda0, weights, Acons, bcons, regularization=1.0e-13, edge_classes=edge_classes,
            )
            ...
            if score < best_score:
                best_score = score
                best = (A, adjacency, edges, edge_index, local_conds, local_sigmins, lambda0, lambdas, distances, corr_stats, patches_tmp, meta, edge_is_added, node_is_flagged)
            if score <= 5.0e-7:
                break
        A, adjacency, edges, edge_index, local_conds, local_sigmins, lambda0, lambdas, distances, corr_stats, patches, meta, edge_is_added, node_is_flagged = best
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="CCPFM / WLS Poisson validation on irregular 2D geometries.")
    ...
    p.add_argument("--outdir", type=str, default="/mnt/data/ccpfm_poisson_results")
    ...
    p.add_argument("--selective-enrichment-neighbors", type=int, default=1, help="Very sparse rescue edges added only for flagged interior-core nodes in CCPFm.")
        ...
        for fut in as_completed(fut_map):
            ...
            try:
                ...
                print(
                    ...
                    f"N={row['n_points']:4d} L2={row['l2']:.3e} Linf={row['linf']:.3e}"
                )
    ...
    for r in sorted([a for a in agg_rows if a['operator'] == 'ccpfm'], key=lambda z: z['domain']):
        if float(r['h_target']) == min(args.hs):
            print(
                ...
                f"added corr p90={r.get('mean_added_edge_correction_rel_p90', float('nan')):.2f}"
            )
