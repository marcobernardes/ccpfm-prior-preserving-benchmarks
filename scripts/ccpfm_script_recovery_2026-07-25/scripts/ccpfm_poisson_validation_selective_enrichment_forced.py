...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--force-min-flagged-core-nodes", type=int, default=2, help="If the absolute enrichment triggers flag too few deep-core nodes, force-flag at least this many worst-scoring core nodes so rescue-edge enrichment is exercised.")
    ...
    for r in sorted([a for a in agg_rows if a['operator'] == 'ccpfm'], key=lambda z: z['domain']):
        if float(r['h_target']) == min(args.hs):
            print(
                ...
                f"added corr p90={r.get('mean_added_edge_correction_rel_p90', float('nan')):.2f}, attempt={r.get('mean_selected_attempt_idx', float('nan')):.2f}"
            )

    print("\nSelective-enrichment activation summary (mean counts at finest h):")
