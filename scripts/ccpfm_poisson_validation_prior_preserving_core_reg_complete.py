...
def _subset_value_stats(vals: Array, subset: Array) -> Dict[str, float]:
    if subset is None or np.sum(subset) == 0:
        return {
            'median': float('nan'),
            'p90': float('nan'),
    ...
    vv = np.asarray(vals[subset], dtype=float)
    return {
...
def solve_ccpfm_correction(
    ...
    def empty_stats() -> Dict[str, float]:
        return {
            ...
            "core_edge_fraction": float('nan'),
            "boundary_edge_fraction": float('nan'),
            "original_core_edge_fraction": float('nan'),
            "original_boundary_edge_fraction": float('nan'),
            "effective_weight_original_edge_median": float('nan'),
            "effective_weight_original_edge_p90": float('nan'),
            "effective_weight_core_edge_median": float('nan'),
            "effective_weight_core_edge_p90": float('nan'),
            "effective_weight_boundary_edge_median": float('nan'),
            "effective_weight_boundary_edge_p90": float('nan'),
            "effective_weight_original_core_edge_median": float('nan'),
            "effective_weight_original_core_edge_p90": float('nan'),
            "effective_weight_original_boundary_edge_median": float('nan'),
            "effective_weight_original_boundary_edge_p90": float('nan'),
            "effective_weight_added_edge_median": float('nan'),
            "effective_weight_added_edge_p90": float('nan'),
            "boundary_band_live": 0.0,
        }
    ...
    if edge_classes is not None:
        ...
        w_orig_stats = _subset_value_stats(weights_eff, edge_is_original)
        w_core_stats = _subset_value_stats(weights_eff, edge_is_core)
        w_bnd_stats = _subset_value_stats(weights_eff, edge_touches_boundary_band)
        w_add_stats = _subset_value_stats(weights_eff, edge_is_added)
        w_orig_core_stats = _subset_value_stats(weights_eff, orig_core)
        w_orig_bnd_stats = _subset_value_stats(weights_eff, orig_bnd)
        ...
        stats.update({
            ...
            "core_edge_fraction": float(np.sum(edge_is_core) / n_all),
            "boundary_edge_fraction": float(np.sum(edge_touches_boundary_band) / n_all),
            "original_core_edge_fraction": float(np.sum(orig_core) / n_all),
            "original_boundary_edge_fraction": float(np.sum(orig_bnd) / n_all),
            ...
            "effective_weight_original_edge_median": w_orig_stats['median'],
            "effective_weight_original_edge_p90": w_orig_stats['p90'],
            "effective_weight_core_edge_median": w_core_stats['median'],
            "effective_weight_core_edge_p90": w_core_stats['p90'],
            "effective_weight_boundary_edge_median": w_bnd_stats['median'],
            "effective_weight_boundary_edge_p90": w_bnd_stats['p90'],
            "effective_weight_original_core_edge_median": w_orig_core_stats['median'],
            "effective_weight_original_core_edge_p90": w_orig_core_stats['p90'],
            "effective_weight_original_boundary_edge_median": w_orig_bnd_stats['median'],
            "effective_weight_original_boundary_edge_p90": w_orig_bnd_stats['p90'],
            "effective_weight_added_edge_median": w_add_stats['median'],
            "effective_weight_added_edge_p90": w_add_stats['p90'],
            "boundary_band_live": float((boundary_band_prior_anchor != 1.0) and np.any(edge_touches_boundary_band)),
...
def aggregate_rows(rows: List[Dict[str, float]]) -> Tuple[List[Dict[str, float]], List[Dict[str, float]]]:
    ...
    for key, grp in sorted(groups.items()):
        ...
        keys_to_average = [
            ...
            "core_edge_fraction", "boundary_edge_fraction", "original_core_edge_fraction", "original_boundary_edge_fraction", "boundary_band_live",
            "effective_weight_original_edge_median", "effective_weight_original_edge_p90",
            "effective_weight_core_edge_median", "effective_weight_core_edge_p90",
            "effective_weight_boundary_edge_median", "effective_weight_boundary_edge_p90",
            "effective_weight_original_core_edge_median", "effective_weight_original_core_edge_p90",
            "effective_weight_original_boundary_edge_median", "effective_weight_original_boundary_edge_p90",
            "effective_weight_added_edge_median", "effective_weight_added_edge_p90",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(
        "[ccpfm-config] "
        f"orig={float(args.original_prior_anchor):g}, "
        f"core={float(args.core_prior_anchor):g}, "
        f"band={float(args.boundary_band_prior_anchor):g}, "
        f"added={float(args.added_prior_anchor):g}, "
        f"reg={float(args.constraint_regularization):.1e}, "
        f"freeze_attempt={int(args.freeze_ccpfm_attempt)}, "
        f"sel_enrich={int(args.selective_enrichment_neighbors)}"
    )
            ...
            try:
                ...
                if row["operator"] == "ccpfm":
                    extra_msg = (
                        ...
                        f" bandlive={int(round(float(row.get('boundary_band_live', 0.0))))}"
                    )
                print(
    ...
    for r in sorted([a for a in agg_rows if a['operator'] == 'ccpfm'], key=lambda z: z['domain']):
        if float(r['h_target']) == min(args.hs):
            band_live = bool(float(r.get('mean_boundary_band_live', 0.0)) > 0.5)
            print(
                f"{r['domain']:>10s}: h={r['h_target']:.4f}, median L2={r['median_l2']:.3e}, "
                f"median Linf={r['median_linf']:.3e}, global order(L2, median)={r['global_order_l2_median']:.3f}, "
                ...
                f"added frac={r.get('mean_added_edge_fraction', float('nan')):.3f}, boundary frac={r.get('mean_boundary_edge_fraction', float('nan')):.3f}, "
                f"orig corr p90={r.get('mean_original_edge_correction_rel_p90', float('nan')):.2f}, "
                f"orig-core corr p90={r.get('mean_original_core_edge_correction_rel_p90', float('nan')):.2f}, "
                f"boundary corr p90={r.get('mean_boundary_edge_correction_rel_p90', float('nan')):.2f}, "
                f"orig-boundary corr p90={r.get('mean_original_boundary_edge_correction_rel_p90', float('nan')):.2f}, "
                f"effW(boundary) p90={r.get('mean_effective_weight_boundary_edge_p90', float('nan')):.2e}, "
                f"attempt={r.get('mean_selected_attempt_idx', float('nan')):.2f}, "
                f"band-live={'YES' if band_live else 'NO'}"
            )
    ...
    for r in sorted([a for a in agg_rows if a['operator'] == 'ccpfm' and float(a['h_target']) == min(args.hs)], key=lambda z: z['domain']):
        band_live = bool(float(r.get('mean_boundary_band_live', 0.0)) > 0.5)
        print(
            f"  {r['domain']:>8s}: boundary frac={r.get('mean_boundary_edge_fraction', float('nan')):.3f}, "
            f"orig-boundary frac={r.get('mean_original_boundary_edge_fraction', float('nan')):.3f}, "
            f"boundary corr p90={r.get('mean_boundary_edge_correction_rel_p90', float('nan')):.2f}, "
            f"orig-boundary corr p90={r.get('mean_original_boundary_edge_correction_rel_p90', float('nan')):.2f}, "
            f"effW(boundary) med={r.get('mean_effective_weight_boundary_edge_median', float('nan')):.2e}, "
            f"effW(boundary) p90={r.get('mean_effective_weight_boundary_edge_p90', float('nan')):.2e}, "
            f"band-live={'YES' if band_live else 'NO'}"
        )
