...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--base-results", required=True,
                   help="CSV or directory containing WLS and frozen CCPFM case rows.")
    p.add_argument("--namls-results",
                   help="CSV or directory containing NAMLS rows. Omit only if base-results already contains NAMLS.")
    p.add_argument("--livegate-results", required=True,
                   help="CSV or directory containing live-boundary CCPFM case rows.")
    p.add_argument("--sparsecore-results", required=True,
                   help="CSV or directory containing forced sparsecore CCPFM case rows.")
    p.add_argument("--selective-results", required=True,
                   help="CSV or directory containing selective sparsecore CCPFM case rows.")
...
def load_all(args: argparse.Namespace) -> list[Row]:
    ...
    if args.namls_results:
        specs.append(("namls", args.namls_results))
    rows: list[Row] = []
    seen_files: set[tuple[Path, str]] = set()
    for role, value in specs:
        path = resolve_csv(value)
        key = (path, role)
        if key in seen_files:
            continue
        seen_files.add(key)
        rows.extend(read_rows(path, role))
    # If base already had NAMLS, retain it. If a dedicated NAMLS file uses operator=namls,
    # classify_method identifies it correctly regardless of role.
    return rows
...
def plot_livegate_gain(rows: list[Row], out: Path, dpi: int) -> None:
    keys = (
        ("l2", "$L_2$"), ("linf", "$L_\\infty$"),
        ("l2_near_boundary_interior", "Near"), ("l2_core_interior", "Core"),
        ("patch_quad_max", "Patch"),
    )
    ...
    for j, (key, label) in enumerate(keys):
        ...
        for domain in DOMAINS:
            h = finest_h(rows, domain)
            vals.append(percent_delta(med(rows, "CCPFM-livegate", domain, key, h), med(rows, "CCPFM-frozen", domain, key, h)))
        ax.bar(x + (j - 2) * width, vals, width, label=label)
    ax.axhline(0.0, linewidth=0.8)
    ax.set_xticks(x, [d.title() for d in DOMAINS])
    ax.set_ylabel("Change relative to frozen CCPFM (%)\nnegative = improvement")
    ax.grid(True, axis="y", alpha=0.25)
